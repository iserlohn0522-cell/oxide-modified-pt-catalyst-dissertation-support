...
from pt_tem_cv.screening import expand_augmentation_screen
from pt_tem_cv.training import train_detector


def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(description="Run or list a materialization-time image-augmentation screen.")
...
def main() -> None:
    ...
    runs = expand_augmentation_screen(args.config)
    if args.list_only:
        print(json.dumps([asdict(run) for run in runs], indent=2))
        return
    for run in runs:
        run_dir = args.output_root / run.family / run.augment_name
